...
export const MegaMenu = ({ categories }: { categories: CategoriesType[] }) => {
  //State to store the selected/active menu and submenu IDs
  const [activeMenu, setActiveMenu] = useState<{
    menu: CategoriesType;
    subMenu: CategoriesType;
  }>({
  ...
  return (
    ...
    >
      ...
      <ul className="flex flex-col space-y-2 text-white submenu-nav min-w-[217px] z-10 pr-2">
        {categories?.map((menu: CategoriesType) => (
          <li
            key={'list' + menu.id}
      ...
      <ul className="flex flex-col space-y-2 text-white submenu-nav min-w-[217px] border border-x-2 border-[#F5F5F5] px-2 border-y-0 ">
        ...
        {categories
          ?.find((menu: CategoriesType) => menu?.id === activeMenu?.menu.id)
          ?.child_categories?.map((subMenu: CategoriesType) => (
            <li
              key={subMenu.id}
      ...
      <ul className="flex flex-col space-y-2 text-white submenu-nav min-w-[217px] px-2">
        ...
        {categories
          ...
          ?.child_categories?.find(
            (subMenu: CategoriesType) => subMenu?.id === activeMenu?.subMenu?.id,
          )
          ?.child_categories?.map((subMenu: CategoriesType) => (
            <li
              key={subMenu.id}
