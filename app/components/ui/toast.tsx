...
import { SuccessAlert } from '../icons/alert';
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 text-white transition-opacity focus:outline-none focus:ring-0 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600 dark:text-slate-50/50 dark:hover:text-slate-50',
      className,
    )}
    toast-close=""
    {...props}
...
function displayToast({
  ...
  return toast({
    className: cn(
      `inset-x-0 bottom-0 flex fixed w-auto rounded-none border-0 text-white py-3 px-4 pr-6 ${type === 'success' ? "bg-semantic-success-500" : "bg-semantic-danger-500"}`,
    ),
    action: (
