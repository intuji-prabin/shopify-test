...
export default function ComparisonWrapper(
  productResponse: finalProductResponse,
) {
...
export function ProductDetailDiv({
  productDetails,
  clearSelectedProduct,
}: {
  productDetails: ProductFinalResponse;
  clearSelectedProduct: (productId: string) => void;
}) {
  return (
      ...
      {productDetails ? (
        <>
          <figure className="relative flex items-center justify-center h-48 p-5 bg-grey-25">
            <img
              src={productDetails?.featuredImage || DEFAULT_IMAGE?.IMAGE}
              alt="product2"
          ...
          <div className="pt-3">
            <p className="text-lg font-medium leading-[22px] text-grey-900 overflow-y-hidden h-11 line-clamp-2">
              <Link
                to={`/product/${productDetails?.handle}`}
          ...
          <div className="pt-2 pb-6">
            <Price
              ...
              price={productDetails?.companyPrice}
              ...
              {productDetails?.companyPrice ? (
                <Form method="post">
                  ...
                  <input
                    ...
                    value={productDetails?.productVariantId}
                  />
                  <input
                    type="hidden"
                    name="quantity"
                  ...
                  <input
                    type="hidden"
                    name="selectUOM"
                    value={productDetails?.selectedUOM}
                  />
