...
export const action = async ({ request, context }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (passwordRecover?.customerRecover?.customerUserErrors.length == 0) {
      ...
      return json({ status: "OK", email: result.data.email }, {
        headers: [
      ...
      setErrorMessage(messageSession, passwordRecover?.customerRecover?.customerUserErrors[0].message);
      return json({}, {
        headers: [
          ['Set-Cookie', await messageCommitSession(messageSession)],
        ],
...
export default function LoginPage() {
  ...
  return (<div className="md:w-[398px] w-full">
    <div className="flex flex-col p-8 space-y-8 bg-white shadow-3xl">
      <div className="flex flex-col items-center justify-center gap-y-5">
        <h4>Forgot Password?</h4>
      </div>
      {data?.status ? <Alert className='border-0 rounded-none bg-semantic-info-100 before:content-[""] before:bg-semantic-info-500 before:inline-block before:h-full before:absolute before:w-1 before:left-0 before:top-0 py-2.5 [&>svg]:top-1/2 [&>svg]:-translate-y-1/2 [&>svg]:left-3'>
        <AlertDescription className="text-base !translate-y-0">
          We’ve sent the password reset instructions to your email ({data.email}) if it has been registered with us.
        </AlertDescription>
      </Alert> : <ForgetPasswordForm />}
    </div>
